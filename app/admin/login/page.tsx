...
export default async function AdminLoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-10 sm:px-6">
      <div className="lift-hover rounded-card border border-bg-border bg-bg-surface p-6 shadow-card">
        <h1 className="text-2xl font-bold text-t-primary">Panel de administración</h1>
        ...
        <form action={loginAdminAction} className="mt-5 space-y-4">
          <label className="block space-y-1">
            <span className="text-sm font-medium text-t-secondary">Correo</span>
            <input
              type="email"
              name="email"
              required
              ...
              className="h-11 w-full rounded-xl border border-bg-border px-3 text-sm text-t-primary placeholder:text-t-muted focus:border-accent focus:outline-none"
            />
          </label>

          <label className="block space-y-1">
            <span className="text-sm font-medium text-t-secondary">Contraseña</span>
            <input
              type="password"
              name="password"
              required
